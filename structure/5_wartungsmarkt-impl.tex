...
Als Proof-of-Concept für eine \acs{B2B}-Anwendung, basierend auf einer Permissioned Blockchain, erfolgt die Entwicklung eines automatisierten sowie dezentralisierten Wartungsmarktes. Dies wird in den folgenden Kapiteln beschrieben.

\section{Konzept und Anforderungen}
Teilnehmer an dem dezentralen Wartungsmarkt sind multiple Unternehmen und Wartungsanbieter. Erstere besitzen \acs{IoT}-Geräte, welche erkennen können, dass sie eine Wartung benötigen. Die Wartungsanbieter erhalten die Informationen zur Wartung und können sich für diese anmelden. Anschließend würden sie diese durchführen und dabei die Wartungsschritte loggen.  
...
Neben den im Kapitel \ref{sec:general-requirements} genannten allgemeinen Anforderungen an \acs{B2B}-Blockchain-Anwendungen, ergeben sich speziellere für das zu entwickelnde System. Die Spezifizierung der Anforderungen ist wichtig, damit eine Blockchain-Plattform ausgewählt und auf verschiedene Probleme analysiert werden kann. Folgende Anforderungen ergeben sich für den dezentralen Wartungsmarkt:
...
\paragraph{Private Preisabsprachen}
Über die Blockchain soll dokumentiert werden, auf welchen Preis sich ein Unternehmen und ein Wartungsanbieter für eine Wartung geeinigt haben. Diese Preisabsprache soll aber nur für die beiden ersichtlich sein.
...
Im folgenden Kapitel werden die Grundlagen von Hyperledger Fabric sowie dem dazugehörigen Framework Hyperledger Composer erläutert. Dies erlaubt ein besseres Verständnis für die nachfolgende Beschreibung der Entwicklung mit den Technologien.

\subsection{Hyperledger Fabric}
\label{subsec:hyperledger-fabric-basics}
Hyperledger Fabric ist eine Blockchain-Plattform für Business-Netzwerke. Es ist modular ausgelegt (siehe z.~B. austauschbare Konsensmechanismen) um es einfach zu erweitern und somit für möglichst viele Use-Cases nutzbar machen zu können \cite{HyperledgerFabricTeamHyperledgerWhitepaper2016}. Im Folgenden wird das grundlegende Konzept von Fabric erklärt.

\paragraph{Chaincode}
Fabric erlaubt den Teilnehmern das Erstellen, Interagieren und Nachverfolgen von digitalen Assets. Diese bestehen letztendlich aus Ansammlungen von Key-Value-Paaren. Für die Interaktion mit diesen werden Transaktionen genutzt. Die Assets und Transaktionen sind u. a. im Chaincode definiert. Dieser ist bei den Nodes im Netzwerk installiert \cite{SchererPerformanceScalabilityBlockchain2017}. Da der Chaincode Programmlogik abbildet, kann er auch als Smart Contract bezeichnet werden \cite{HyperledgerFabricTeamChaincodeHyperledgerFabric}.
...
In Hyperledger Fabric gibt es Clients, Peer Nodes und Ordering Nodes. Erstere interagieren mit der Blockchain, indem sie Transaktionen über ihre Identität ausführen. Die Peer Nodes haben 2 mögliche Rollen: Als Endorser simulieren sie Transaktionen, und als Committer validieren und hängen sie Blöcke (und die darin enthalten Transaktionen) an die lokale Blockchain an. Jeder Endorser ist gleichzeitig ein Committer. Die Ordering Nodes sortieren die Transaktionen nach First-Come-First-Serve Prinzip und erstellen daraus einen Block \cite{SchererPerformanceScalabilityBlockchain2017}\cite{HyperledgerFabricTeamHyperledgerFabricGlossary}.

Der Transaktionsfluss zwischen diesen wird anhand der Abbildung \ref{fig:transaction-flow} erklärt. In einem Fabric Netzwerk einigen sich die Unternehmen auf den zu nutzenden Chaincode für eine Anwendung und speichern ihn in der Blockchain. Anschließend erstellen Clients auszuführende Transaktionen und senden diese an die Endorser (Schritt 1). Sie simulieren die Ausführung der Transaktionen (Schritt 2). Dazu führen sie die Transaktion auf der State Database aus, um die Datenänderungen zu erkennen. Diese werden jedoch noch nicht festgeschrieben. Der Client erhält die gültigen Transaktionen zurück (Schritt 3), und übermittelt sie an eine Ordering Node (Schritt 4). Diese sortiert die Transaktionen, erstellt einen Block und sendet sie an die Committer (Schritt 5). Die Committer validieren die Transaktionen, hängen den Block an die Blockchain an und führen die bereits simulierten Datenänderungen sequentiell in der State Database durch (Schritt 6). Dabei werden in Konflikt stehende Transaktionen erkannt und als invalide gekennzeichnet \cite{SchererPerformanceScalabilityBlockchain2017}.

\begin{figure}[!htb]
    \centering
      \includegraphics[width=0.9\textwidth,angle=0]{images/fabric-transaction-flow}
       \caption{Transaktionsfluss von Hyperledger Fabric \cite{SchererPerformanceScalabilityBlockchain2017}.}
...
In der \acs{BND} bestehen drei Typen von Assets. Der \textit{MachineStatus} gehört zu einer Maschine und gibt an, ob sie funktionstüchtig ist. Der \textit{MaintenanceContract} enthält u. a. Informationen über den Wartungsgrund und den Status der Wartung. Letztendlich gibt es noch die \textit{PaymentAgreement}. Diese dokumentiert die abgesprochene Bezahlung zwischen Unternehmen und Wartungsanbieter für einen bestimmten Wartungsvertrag. Listing \ref{lst:maintenance-contract} zeigt die Definition eines \textit{MaintenanceContract}.
